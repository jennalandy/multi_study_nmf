...
discovery_sampler <- function(
  ...
  if (!fixed) {
    A <- array(0,dim=c(S,Ntot))
    A[,1:Ntot] <- rbinom(S*Ntot,1,0.5)
    A[rowSums(A)==0,1] <- 1
  } else {
    A <- A.fixed
  }
  ...
  P <- P+min(P[P!=0])
  E <- lapply(1:S,function(s) array(rgamma(N*G[s],alpha_e[[s]],beta_e[[s]]),dim=c(N,G[s])))
  E <- lapply(1:S,function(s) E[[s]]+min(E[[s]][E[[s]]!=0]))
  if (!is.null(inits)) {
    P <- inits[[1]]
    E <- inits[[2]]
  } 
  ...
  for (l in 1:totaliters) {
    ...
    for (s in 1:S) {
      for (n in 1:N) {
        for (j in 1:G[s]) {
          beta_e[[s]][n,j] <- sample_betanjse(n,j,s,E[[s]],alpha_e,a_e[s],b_e[s],gamma)
        }
      }
    }
